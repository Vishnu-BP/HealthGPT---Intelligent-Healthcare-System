...
def insert_contact(name, email, contact, message):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    query = f""" INSERT INTO CONTACT (NAME, EMAIL, CONTACT, MESSAGE)
...
def insert_patients(name, patient_id, email, contact, country, state, age, gender, disease, result, pin):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    query = f"""
        INSERT INTO PATIENTS
        (NAME, PATIENT_ID, EMAIL, CONTACT, COUNTRY, STATE, AGE, GENDER, DISEASE, RESULT, PIN)
        VALUES ('{name}', '{patient_id}', '{email}', '{contact}', '{country}', '{state}', '{age}', '{gender}', '{disease}', '{result}', '{pin}')
    """
    cursor.execute(query)
    conn.commit()
    conn.close()
...
def internal_home():
    if "user" in session:
        return render_template('internal_home.html', login_user=session['user'])
    else:
